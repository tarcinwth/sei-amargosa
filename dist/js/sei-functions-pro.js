...
var iconSeiPro = URL_SPRO + 'icons/menu/seipro.png';
...
// === FUNÇÃO DE SANITIZAÇÃO COM DOMPurify ===
...
function appendDebugReport() {
    ...
    $('div[data-ref="infraAcaoBarraSistema"]').prepend(htmlIconDebug);
}
...
function forceOnLoadBodyPage() {
    ...
    if (typeof $().resizable !== 'undefined' && !$('.sparkling-modal-frame').length) onload();
}
...
const SEI_PRO_EASTER_EGGS = {
    home_loaded: [
        'Se voce estiver vendo isso, para tudo e volta trabalhar',
        'Levanta, toma um cafe, volta. Essa ordem importa.',
        'Isso aqui foi feito por: Tarcio Rodrigues. Manda um salve.',
        'Oi. Tudo bem? So curiosidade mesmo.',
        'Console aberto = modo detetive ativado. Encontrou algo? Vai la.'
    ],
    external_upload: [
        'Arquivo chegou. Guga aprova.',
        'Anexo enviado. O sistema ficou feliz, pode acreditar.',
        'Upload feito com sucesso. Missao cumprida, guerreiro.',
        'Arquivo recebido. Agora sim, tava esperando isso.'
    ],
    config_open: [
        'Configuracoes abertas. Hora de olhar tudo.',
        'Painel aberto. Mexa no que quiser, a gente aguenta, la ele.',
        'Opa, configurando as coisas? Respeito.',
        'Esse design foi feito pra nao parecer uma tela de banco. Conseguimos?'
    ],
    config_saved: [
        'Salvo. Pode fechar sem medo agora.',
        'Configuracoes salvas. O SEI Pro agradece com todo carinho.',
        'Tudo guardado. Sua rotina acabou de melhorar um pouco.',
        'Salvo com sucesso. Isso aqui ta mais seu agora.'
    ]
};

function triggerEasterEgg(name, force = false) {
    if (typeof sessionStorage === 'undefined') return;
    try {
        const seen = JSON.parse(sessionStorage.getItem('seiProEasterEggsSeen') || '{}');
        if (!force && seen[name]) return;
        const list = SEI_PRO_EASTER_EGGS[name] || [];
        if (list.length === 0) return;
        const phrase = list[Math.floor(Math.random() * list.length)];

        if (typeof console !== 'undefined' && typeof console.info === 'function') {
            console.info('%cSEI Pro easter-egg: ' + phrase, 'color:#7c4dff;font-weight:700;');
        }

        if (typeof document !== 'undefined') {
            const originalTitle = document.title;
            document.title = phrase;
            setTimeout(function () { document.title = originalTitle; }, 1800);
        }

        seen[name] = true;
        sessionStorage.setItem('seiProEasterEggsSeen', JSON.stringify(seen));
    } catch (e) {}
}

function initHomeEasterEggs() {
    if (typeof sessionStorage === 'undefined') return;
    try {
        document.body.setAttribute('data-extension-author', 'Tarcio Rodrigues');
    } catch (e) {}

    triggerEasterEgg('home_loaded', true);
}

function setSortDivPanel() {
    if (window.SEIProModules && window.SEIProModules.home && typeof window.SEIProModules.home.setSortDivPanel === 'function') {
        return window.SEIProModules.home.setSortDivPanel();
    }
...
function controleSortDivPanel() {
    if (window.SEIProModules && window.SEIProModules.home && typeof window.SEIProModules.home.controleSortDivPanel === 'function') {
        return window.SEIProModules.home.controleSortDivPanel();
    }
...
function forcePlaceHoldChosen() {
    if (window.SEIProModules && window.SEIProModules.home && typeof window.SEIProModules.home.forcePlaceHoldChosen === 'function') {
        return window.SEIProModules.home.forcePlaceHoldChosen();
    }
...
function setPlaceHoldChosen(this_) {
    if (window.SEIProModules && window.SEIProModules.home && typeof window.SEIProModules.home.setPlaceHoldChosen === 'function') {
        return window.SEIProModules.home.setPlaceHoldChosen(this_);
    }
...
function initChosenReplace(mode, this_ = false, force = false, TimeOut = 9000) {
    if (window.SEIProModules && window.SEIProModules.home && typeof window.SEIProModules.home.initChosenReplace === 'function') {
        return window.SEIProModules.home.initChosenReplace(mode, this_, force, TimeOut);
    }
...
function scrollToElementArvore(id_documento) {
    if (window.SEIProModules && window.SEIProModules.arvore && typeof window.SEIProModules.arvore.scrollTo === 'function') {
        return window.SEIProModules.arvore.scrollTo(id_documento);
    }
...
function updateDadosProcesso(idElement, value, callback = false) {
    if (window.SEIProModules && window.SEIProModules.processo && typeof window.SEIProModules.processo.updateDadosProcesso === 'function') {
        return window.SEIProModules.processo.updateDadosProcesso(idElement, value, callback);
    }
...
function getLinksProcessoAjax(id_procedimento, callback) {
    if (window.SEIProModules && window.SEIProModules.processo && typeof window.SEIProModules.processo.getLinksProcessoAjax === 'function') {
        return window.SEIProModules.processo.getLinksProcessoAjax(id_procedimento, callback);
    }

    if (window.SEIProModules && window.SEIProModules.arvore && typeof window.SEIProModules.arvore.getLinks === 'function') {
        return window.SEIProModules.arvore.getLinks(id_procedimento, callback);
    }
...
function getInteressadosProcesso(txtInteressado, callback) {
    if (window.SEIProModules && window.SEIProModules.processo && typeof window.SEIProModules.processo.getInteressadosProcesso === 'function') {
        return window.SEIProModules.processo.getInteressadosProcesso(txtInteressado, callback);
    }

    if (window.SEIProModules && window.SEIProModules.arvore && typeof window.SEIProModules.arvore.getInteressados === 'function') {
        return window.SEIProModules.arvore.getInteressados(txtInteressado, callback);
    }
...
function setInteressadosSend() {
    if (window.SEIProModules && window.SEIProModules.processo && typeof window.SEIProModules.processo.setInteressadosSend === 'function') {
        return window.SEIProModules.processo.setInteressadosSend();
    }

    if (window.SEIProModules && window.SEIProModules.arvore && typeof window.SEIProModules.arvore.setInteressadosSend === 'function') {
        return window.SEIProModules.arvore.setInteressadosSend();
    }
        ...
        .dialog({
        	...
        	open: function(){
                ...
                if (data.mode == 'nivel_acesso' && (prop.rdoNivelAcesso == '1' || prop.rdoNivelAcesso == '2')) {
                    ...
                    if (listaGruposAcompEsp && listaGruposAcompEsp_unidade == idUnidade) {
...
function waitLoadPro(Obj, ElemRaiz, Elem, func, TimeOut = 6000) {
  ...
  function execute() {
    try {
      ...
      console.warn('waitLoadPro error:', error);
    }
  }

  if (root.length && root.find(Elem).length) {
    execute();
    return;
  }
    ...
    if (TimeOut > 0) {
      setTimeout(function () {
        if (root.length && root.find(Elem).length) {
          ...
          waitLoadPro(Obj, ElemRaiz, Elem, func, TimeOut - 200);
        }
      }, 200);
    }
    return;
  ...
  try {
    observer.observe((Obj[0] || document), { childList: true, subtree: true, attributes: true });
  } catch (error) {
    observer.disconnect();
  }
...
function waitLoadProSimple(Elem, func, TimeOut = 6000) {
  ...
  function execute() {
    try {
      ...
      console.warn('waitLoadProSimple error:', error);
    }
  }
    ...
    if (TimeOut > 0) {
      setTimeout(function () {
        if (Elem.length > 0) {
          ...
          waitLoadProSimple(Elem, func, TimeOut - 200);
        }
      }, 200);
    }
    return;
  ...
  try {
    observer.observe((Elem[0] || document), { childList: true, subtree: true, attributes: true });
  } catch (error) {
    observer.disconnect();
  }
...
function getLinksArvoreAjax(htmlArvore) {
    var html = (typeof htmlArvore === 'string') ? htmlArvore : '';
    if (!html) {
        return [];
    }

    if (typeof linksArvoreCache[html] !== 'undefined') {
        return linksArvoreCache[html];
    }
    ...
    function pushLink(item) {
        ...
        var key = (item.name || '').toLowerCase() + '::' + item.url;
        if (!seen[key]) {
            seen[key] = true;
            links.push(item);
        }
    ...
    try {
        var matches = html.match(/Nos\[0\]\.acoes\s*=\s*'([^']+)'/g) || [];
        if (matches.length === 0 && html.indexOf('NosAcoes[0]') !== -1) {
            var fallback = html.match(/NosAcoes\[0\]\s*=\s*new\s+infraArvoreAcao\("[^"]+"\s*,\s*"([^"]+)"\s*,\s*"([^"]+)"/g) || [];
            $.each(fallback, function (index, value) {
                var parts = value.split(',');
                var name = (parts[1] || '').replace(/"/g, '').trim();
                var url = (parts[2] || '').replace(/"/g, '').trim();
                pushLink({ name: name || 'Ação', url: url });
            });
        }

        $.each(matches, function (index, val) {
            var barraControle = val.replace(/Nos\[0\]\.acoes\s*=\s*'/, '').replace(/'$/, '');
            $('<div>' + barraControle + '</div>').find('a[href], a[href*="controlador.php?acao="], a.botaoSEI').each(function () {
                var href = $(this).attr('href');
                var name = $(this).find('img').attr('title') || $(this).text().trim() || $(this).attr('title') || 'Ação';
                if (typeof href !== 'undefined' && href !== '#' && href !== '') {
                    pushLink({ name: name, url: href });
                }
            });
        });

        if (!links.length) {
            $('<div>' + html + '</div>').find('a[href*="controlador.php?acao="], a.botaoSEI, a[href*="acao="]').each(function () {
                var href = $(this).attr('href');
                var name = $(this).find('img').attr('title') || $(this).text().trim() || $(this).attr('title') || 'Ação';
                if (typeof href !== 'undefined' && href !== '#' && href !== '') {
                    pushLink({ name: name, url: href });
        ...
        console.warn('getLinksArvoreAjax error:', error);
    }

    linksArvoreCache[html] = links;
    return links;
}
...
const appendAutocompleteProc = (this_, elem) => {
...
function getDataRecebimentoPro(listAndamento, listProc = false, acompanhamentoEsp = '') {
    ...
    $.each(listAndamento.andamento,function(j, e){
        if (siglaUnidadeAtual == e.unidade && e.descricao.indexOf('Processo remetido pela unidade') !== -1) {
            datesend = e.datahora;
    ...
    $.each(listAndamento.andamento,function(i, v){
        ...
        if (siglaUnidadeAtual == v.unidade && (v.descricao == 'Processo recebido na unidade' || v.descricao == 'Reabertura do processo na unidade')) {
            ...
            return false;
        } else if (siglaUnidadeAtual == v.unidade && (v.descricao == 'Processo p\u00FAblico gerado' || v.descricao.indexOf('Processo restrito gerado') !== -1 )) {
            dataRecebimento.push({id_procedimento: listAndamento.id_procedimento, processo: listAndamento.processo, datahora: v.datahora, unidade: v.unidade, descricao: v.descricao, datetime: datevisit, datesend: datesend, descricaosend: descricaosend, unidadesend: unidadesend, unidadesendfull: unidadesendfull, datageracao: datageracao, descricaodatageracao: descricaodatageracao, observacoes: observacoes, acompanhamentoesp: acompanhamentoesp});
            return false;
        }
    });
...
const getImageBase64FromImgElement = async (imgElement) => {
    const img = new Image();
...
function getHolidaysBr(y) {
    ...
    var conscienciaNegra = moment('20/11/'+y,'DD/MM/YYYY');
