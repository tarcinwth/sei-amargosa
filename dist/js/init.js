...
$.getScript(getUrlExtension("js/lib/moment.min.js")).done(function () {
    if (typeof moment !== 'undefined' && typeof moment.duration === 'function' && typeof moment.duration().format !== 'function') {
        $.getScript(getUrlExtension("js/lib/moment-duration-format.min.js"));
    }
}).fail(function () {
    if (typeof console !== 'undefined' && console.warn) {
        console.warn('SEI Pro: não foi possível carregar Moment.js no startup.');
    }
});
...
function initProModules() {
    if (window.__seiModuleInitRunning) return;

    window.__seiModuleInitRunning = true;
    try {
        const modules = window.SEIProModules || {};
        ['home', 'arvore', 'processo'].forEach(function (name) {
            if (modules[name] && typeof modules[name].init === 'function') {
                modules[name].init();
            }
        });
    } finally {
        window.__seiModuleInitRunning = false;
    }
}

function scheduleModuleInit() {
    if (window.__seiModuleInitScheduled) return;

    window.__seiModuleInitScheduled = true;
    initProModules();

    if (typeof MutationObserver !== 'undefined' && (document.documentElement || document.body) && !window.__seiModuleObserver) {
        const observer = new MutationObserver(function () {
            if (!window.__seiModuleInitRunning) {
                initProModules();
            }
        });
        observer.observe(document.documentElement || document.body, { childList: true, subtree: true, attributes: true });
        window.__seiModuleObserver = observer;
    }
